...
class UrlHandler(BaseCRUD):
    ...
    async def get_user_recent_urls(self, user_id: str | ObjectId, limit: int = 5):
        return ListUrl(
            urls=await self._db_conn.get_collection(self._collection)
    ...
    async def _is_owner(self, short_url: str, user_id: str) -> bool:
        res = await self._db_conn.get_collection(self._collection).find_one(
    ...
    async def shorten_url(
        self, user_id: str | ObjectId, original_url: str, custom_alias: str | None = None, **kwargs
    ):
        if custom_alias:

...
class UrlHandler(BaseCRUD):
    ...
    async def get_user_urls(self, user_id: str):
        return await self._db_conn.get_collection(self._collection).find({"user_id": user_id}).to_list(None)
    ...
    async def shorten_url(self, user_id: str | ObjectId, original_url: str):
        short_url = await self._generate_short_url(original_url)
        await self.create({"user_id": user_id, "original_url": original_url, "short_url": short_url})
        return short_url
